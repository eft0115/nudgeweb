...
description: Syncs ruleRelationships.js data with actual .Codex/ file structure and updates Storybook rule visualization. Run pnpm generate-rules instead for automated sync.
...
```
1. 룰/스킬 변경 ──→ 2. 데이터 동기화 ──→ 3. AGENTS.md 반영 ──→ 4. 검증 안내
```
...
1. 대상 파일 수정/추가/삭제
2. 변경 유형 파악:
   - 룰 추가 → `.Codex/rules/` 에 파일 생성
   - 룰 삭제 → 파일 제거 + 참조 정리
   - 스킬 추가 → `.Codex/skills/{name}/SKILL.md` 생성
...
```js
// 노드
{ id: 'rule-id', name: 'rule-name.md', priority: 'MUST', path: '.Codex/rules/...', description: '설명' }
...
| 섹션 | 내용 |
|------|------|
| 룰 계층 구조 | AGENTS.md → 서브룰 트리 다이어그램 |
